...
ACBarrierFunction::computeDFDOP(PFFunctionType type)
{
  switch (type)
  {
    ...
    case Jacobian:
    {
      ...
      return (_d2mudvar2[_qp] * calculate_f0() + _dmudvar[_qp] * df0dvar) * _phi[_j][_qp];
    } // case Jacobian:
...
ACBarrierFunction::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  df0deta_base = (*_vals[j])[_qp] * (*_vals[j])[_qp] - 1.0 + 2.0 * _gamma[_qp] * (_u[_qp] * _u[_qp] + sum_etai2);
  df0deta  = (*_vals[j])[_qp] * df0deta_base;

  return ((*_d2mudvardeta[j])[_qp] * calculate_f0() + _dmudvar[_qp] * df0deta) * _phi[_j][_qp] * _test[_i][_qp] * _L[_qp];
} // Real ACBarrierFunction::computeQpOffDiagJacobian(unsigned int jvar)

Real
ACBarrierFunction::calculate_f0()
{
  ...
  for (unsigned int i = 0; i < _n_eta; ++i)
  {
    ...
    eta_phase += 0.25 * (*_vals[i])[_qp] * (*_vals[i])[_qp] * (*_vals[i])[_qp] * (*_vals[i])[_qp] - 0.5 * (*_vals[i])[_qp] * (*_vals[i])[_qp];
    eta_interface += (_u[_qp] * _u[_qp] + sum_eta_j2) * (*_vals[i])[_qp] * (*_vals[i])[_qp] * _gamma[_qp];
  } // for (unsigned int i = 0; i < _n_eta; ++i)
